    ...
    switch (event.type) {
      ...
      case SDL_EVENT_MOUSE_BUTTON_DOWN:
        if (event.button.down == true) {
          state->fRects.push_back() = SkRect::MakeLTRB(SkIntToScalar(event.button.x), SkIntToScalar(event.button.y),
...
int main(int argc, char** argv) {
  ...
  sk_sp<SkSurface> surface(SkSurfaces::WrapBackendRenderTarget(grContext.get(), target, kBottomLeft_GrSurfaceOrigin,
