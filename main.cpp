...
int main(int argc, char *argv[]) {
    ...
    auto top_bar = controller.AddObject(
        std::make_unique<UIGroup>(UIAttributes{0.0, 0.0, 1.0, 0.04}, "top_bar"));
    ...
    auto content_area = controller.AddObject(
        std::make_unique<UIGroup>(UIAttributes{0.0, 0.04, 1.0, -1}, "content_area"));
    //
    // 内容区左侧面板
    auto left_panel = controller.AddObject(
        std::make_unique<UIGroup>(UIAttributes{0.0, 0.0, 0.1, -1}, "left_panel"),
        content_area);
    controller.AddObject(
        std::make_unique<Button>(UIAttributes{0.02, 0.1, 0.05, 0.5},
                                 "left_panel_button1",
                                 SDL_Color{200, 200, 0, 255},
                                 0.5),
        left_panel); // 左侧菜单项1
    controller.AddObject(
        std::make_unique<Button>(UIAttributes{0.02, 0.2, 0.05, 0.5},
                                 "left_panel_button2",
                                 SDL_Color{0, 200, 200, 255},
                                 0.5),
        left_panel); // 左侧菜单项2
    // 底部状态栏区域
    auto bottom_bar = controller.AddObject(
        std::make_unique<UIGroup>(UIAttributes{0.0, -0.04, 1.0, -1}, "bottom_bar")); // 使用负y表示从底部开始
    controller.AddObject(
        std::make_unique<Button>(UIAttributes{0.05, 0.01, 0.2, -1},
                                 "bottom_bar_button1",
                                 SDL_Color{255, 150, 50, 255},
                                 0.5),
        bottom_bar); // 状态信息1
    controller.AddObject(
        std::make_unique<Button>(UIAttributes{0.75, 0.01, 0.2, -1},
                                 "bottom_bar_button2",
                                 SDL_Color{50, 255, 150, 255},
                                 0.5),
        bottom_bar); // 状态信息2
